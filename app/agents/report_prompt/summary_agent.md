...
1) headline 작성 원칙
- **oneline_summary**: 인터뷰 로그에서 후보자를 가장 잘 설명하는 핵심 특징 1줄 작성
  - 후보자의 답변 내용, 커뮤니케이션 스타일, 인상적인 경험을 기반으로 작성
  - 숫자·구체적 지표는 로그에 존재할 때만 포함
  - 존재하지 않으면 창작 금지, 행동 기반 표현으로 대체
  - 피상적 형용사 대신 구체적 행동 묘사 중심
  - **예: "복잡한 기술 개념을 비유를 통해 명확하게 설명하는 커뮤니케이터", "실패 경험을 구체적 개선 방안과 함께 설명하는 문제 해결 중심 사고"**

- **tag**: 후보자의 대표 키워드 3~5개 추출
  - 인터뷰 로그에서 반복적으로 나타나는 핵심 역량, 경험, 특징을 키워드화
  - 구체적이고 명확한 단어 사용 (추상적 형용사 지양)
  - 로그에 근거 없는 키워드 생성 금지
  - **예: ["MSA 아키텍처", "팀 리더십", "문제 해결", "기술 문서화", "애자일 경험"]**
...
- **question_type_ratio 필드 규칙**:
  - 질문 유형별 비율을 파악할 수 있으면 QuestionTypeRatio 객체 리스트로 반환
  - 예: [{{"type": "사실관계파악", "ratio": "40%"}}, {{"type": "기술질문", "ratio": "35%"}}]
  - 질문 유형을 파악할 수 없거나 정보가 부족하면 **빈 리스트 []** 반환
  - "정보 없음" 같은 문자열 반환 금지
