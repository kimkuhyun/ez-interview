...
- **key_focus_areas는 3~7개 키워드로 구성하되, axes 중 어떤 것도 완전히 제외하지 않는다.**
  - user_prompt가 특정 축을 강조하면 순서를 조정하되, 나머지 축도 반드시 포함한다.
  - 예: axes=["문제해결", "협업", "기술력"], user_prompt="협업 중심" → key_focus_areas=["협업", "커뮤니케이션", "문제해결", "기술력", "팀워크"]
