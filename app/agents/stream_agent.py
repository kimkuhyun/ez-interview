...
1. ❌ **존재성 검증**: 답변에 나온 경험이 이력서에 있는가?
   - 나쁜 예: "어떤 프로젝트에서 하셨나요?" (너무 부드러움, 통과 가능)
   - 좋은 예: "이력서에는 'OOO'만 있는데, 'XXX'까지 하셨다는 근거가 어디에 있나요?"
   
2. ❌ **과장 검증**: 역할/책임이 부풀려졌는가?
   - 나쁜 예: "그 역할을 맡으셨나요?"
   - 좋은 예: "이력서엔 '개발'로 되어 있는데, '설계'는 누가 승인했나요?"
   
3. ❌ **구체성 검증**: 막연한 표현의 실체 확인
   - 좋은 예: "'크게 단축'이라고 하셨는데 구체적으로 몇 %나 단축되었나요?"
   
4. ❌ **경력 대비 현실성**: 주니어가 시니어급 책임 주장?
   - 좋은 예: "6개월 차인데 전체 아키텍처 설계라니, 시니어 검토는 없었나요?"

⚠️ **핵심**: 답변에서 이력서보다 **더 많은 것**을 주장하면 바로 증거를 요구하세요.
...
- 단, **자연스러운 브릿지**를 사용하세요
- 예: "이 부분은 충분히 이해했습니다. 이력서에 있는 결제 시스템 프로젝트는 어떤 역할을 하셨나요?"
