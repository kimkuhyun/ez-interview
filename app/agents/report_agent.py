    ...
    return ChatPromptTemplate.from_messages(
        [
            (
                ...
                "- 이력서, 채용공고(JD), 인터뷰 로그에서 평가에 필요한 정보를 잘 찾을 수 있도록 질의를 설계합니다.\n"
                "- user_prompt가 있으면 채용담당자가 보고 싶은 관점을 질의에 반영합니다.\n"
                "- 없으면 axes_keys(핵심역량 축)를 기준으로 일반적인 채용 평가용 질의를 만듭니다.\n"
...
def retrieve_contexts(
    ...
    try:
        ...
        for idx, result in enumerate(resume_results, 1):
            ...
            chunk_type = result.get("doc_type", "unknown")
            similarity = result.get("score", "N/A")
    ...
    return ChatPromptTemplate.from_messages(
        [
            (
                ...
                "2. 답변의 깊이 평가 (구체성, 전문성)\n"
                "3. 인터뷰 로그의 신뢰도 판단 (인터뷰 로그가 종합적으로 참고자료로 쓸만한지)\n\n"
                "**점수 기준:**\n"
                "- contradiction_score: 0(매우 일관적) ~ 100(매우 모순적)\n"
                "- depth_score: 0(피상적) ~ 100(매우 깊이 있음)\n"
                "- reliability_score: 0(신뢰 불가) ~ 100(매우 신뢰)\n"
                "- 인터뷰 로그 양이 적거나 모호하면 depth_score와 reliability_score는 보수적으로 낮게 주고,(0점등 과감하게) "
                "final_comment에 \"데이터 제한\"을 언급합니다.\n\n"
                "**디베이트 방식 분석:**\n"
                ...
                "- 10~15턴 정도의 짧은 디베이트로 구성하고, 각 턴은 한 문단 이내로 작성합니다.\n\n"
                "**출력 필드:**\n"
    ...
    return ChatPromptTemplate.from_messages(
        [
            (
                ...
                "사용자 요구사항{user_prompt}에 상세한 핵심역량 계산방법이 들어오면 그것을 반영하고"
                "아래의 평가원칙과 점수기준은 무시하세요\n"
                "**평가 원칙:**\n"
                "1. 실제 증거에 기반 (추측 금지)\n"
                "2. 구체적 성과와 수치 필요\n"
                "3. 증거 약하면 과감하게 낮은 점수(10점 이하도 과감하게 줄것)\n"
                "4. 가중치 합계 정확히 100\n"
    ...
    return ChatPromptTemplate.from_messages(
        [
            (
                ...
                "5. 문장의 맥락을 보고 적절한 JID에 매핑하세요"
                "단순 키워드로 매핑 하지 마세요(예: TOEIC=커뮤니케이션 아님)\n\n"
                "**필수 출력 형식 (CRITICAL):**\n"
    ...
    return ChatPromptTemplate.from_messages(
        [
            (
                ...
                "**평가 항목:**\n"
                "1. 헤드라인(summary, tag)와 인터뷰 분석 점수(모순/깊이/신뢰도)가 서로 논리적으로 맞는지\n"
                "2. 핵심역량 scores/weights가 충분한 근거를 가지고 타당한 점수를 산출 하였는지\n"
                "3. evidence가 핵심역량 평가의 근거를 제대로 뒷받침하는지, 단순 키워드로만 매핑하지 않았는지?\n"
                "4. 인터뷰 요약(overall, qa_summaries, positive, negative, stats)이 원본 길이 대비 충분한지\n"
                "5.  데이터(이력서/JD/로그) 양이 매우 적은데도 리포트가 과도하게 확신에 찬 톤으로 쓰여 있지 않은지\n\n"
                "**품질 기준:**\n"
                "- quality_score: 0.0~1.0\n"
                "- is_sufficient: 0.70 이상이면 True\n"
                "- grade: A+ (0.95+), A (0.85+), B+ (0.75+), B (0.65+), C (0.55+), D/F (0.55 미만)\n"
                "- suggestions: 개선 제안 (예: \"데이터가 부족한 역량에는 낮은 점수와 '근거 부족' 문구를 추가\")\n"
                "- issues: 발견된 문제 리스트 (예: \"모든 역량 점수가 60~70점 사이에만 분포\")\n"
                "- missing_aspects: 빠져 있는 관점/내용 (예: \"JD의 보안 관련 요구사항이 전혀 반영되지 않음\")\n"
                 "- query_hint: 재검색 시 보완해야 할 정보에 대한 간단한 힌트 문자열 또는 JSON\n"
            ),
            (
                "human",
                "[원본 데이터 요약]\n"
